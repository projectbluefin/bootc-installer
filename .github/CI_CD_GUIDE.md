...
| GUI deletes the temporary recipe after install attempt | Recipe cleanup is part of handling plaintext passphrases safely | `bootc_installer/views/progress.py` | Before/after `find ~/.cache/bootc-installer -maxdepth 1 -name 'bootc-recipe-*.json'` |
